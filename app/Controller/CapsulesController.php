...
/*			
			$this->Capsule = new CapsuleRepository('mongo');
			if (strcmp($_GET['dest'], 'mysql') == 0) {
				$this->Capsule = new CapsuleRepository($_GET['dest']);
			}
*/			
//MUTE			$this->Capsule->fetchPage(0, 10, 0);
...
//			$this->render('index.ctp');
		...
		public function doListByJson() {
			...
			if ($this->request->query("page") && $this->request->query("limit")) {
				$aggregate = $this->Capsule->fetchPage($this->request->query("page"), $this->request->query("limit"));				
			} else {
				$aggregate = $this->Capsule->find();
			}
