...
//	include "../Repository/Sport/MySQLRepository";
...
//				($tags != null) ? $value['tags'] = json_encode($tags, JSON_UNESCAPED_UNICODE) : $value['tags'] = null;
		...
		public function findById() {
			...
			if (strcmp($dest, 'mysql') == 0) {
				$this->loadModel('Sport');
				$details = $this->Sport->query("SELECT * FROM sport_details WHERE sport_id = $sportId");
				$sports = $this->Sport->findById($_GET['sport_id']);
				$reply = '[{';
				$reply .= '"' . 'id' . '"' . ': ' . $sports[0]['Sport']['sport_id'] . ', ';
				$reply .= '"' . 'name' . '"' . ': ' . '"' . $sports[0]['Sport']['name'] . '"' . ', ';
				
				$reply .= '"' . 'details' . '": ' . '[';
				for ($i = 0; $i <= count($details) - 1; $i++) {
					if (1 <= $i) $reply .= ', ';
					$reply .= '{';
					$reply .= '"' . 'id' . '"' . ': ' . $details[$i]['sport_details']['sport_detail_id'] . ', ';
					$reply .= '"' . 'body' . '"' . ': ' . '"' . $details[$i]['sport_details']['body'] . '"' . ', ';
					$reply .= '}';
				}
				$reply .= ']';
...
//DEL			$this->autoLayout = false;

			$sportId = $this->request->data("id");
			$sportId = $this->request->query("id");
//CakeLog::write('debug', '$sportId: ' . var_dump($this->request->data));
			$this->loadModel('Sport');
				...
				if ($tags != null) {
					...
					foreach ($tags as &$v2) {
						try {
							...
							if (in_array('tag_name', $v2) && $v2['tag_name'] == 'sports') {
								$invoker = $this->Sport;
							} else if (in_array('tag_name', $v2) && $v2['tag_name'] == 'structure') {
								$invoker = $this->Structure;
							}
							if ($invoker == null) throw new Exception();
						...
						array_push($listOfTag, array('id' => $tag[0]['_id']->{'$id'}, 'name' => $tag[0]['name']));

					}
					$value['tags'] = $listOfTag;
				}
				$value['_id'] = $id;
			}
