...
//                    solvent.onstartdrag = (e) => {
                    ...
                    let innerHTML;
                    let unit4Disc = 160;
                    sail = new Sail({rootDiv: discSystem.getBottom(), captionForFront: "river", height: "150px", width: "160px"});
                    sail.jointDiv.style.transform += "translateZ(1px)";


                    innerHTML = `
                        漲: <font size="1px">{m: ". gets released", l: () => </font>{
                            .glide();
                        }}
                    `;
                    sail = new Sail({rootDiv: discSystem.getBottom(), captionForFront: innerHTML, height: "125px", width: "160px"});
                    sail.jointDiv.style.transform += "translateZ(" + (unit * 1 * -1) + "px) translateX(" + (unit4Disc * 1.1) + "px)";


                    let disc;
                    disc = new Disc({rootDiv: discSystem.getBottom(), dur: (unit * 2) + "px", innerHTMLForFrontInner: `slope >>= climb`});
                    disc.frontInnerDiv.addEventListener("click", (e) => {discSystem.getLoupe().innerHTML = e.target.innerHTML;});
                    disc.moveY(0);

                    disc = new Disc({rootDiv: discSystem.getBottom(), dur: (unit * 2) + "px", innerHTMLForFrontInner: `漲 << chargeE`});
                    disc.frontInnerDiv.addEventListener("click", (e) => {discSystem.getLoupe().innerHTML = e.target.innerHTML;});
                    disc.moveY(unit * 1);
                    disc.moveX(10);

                    disc = new Disc({rootDiv: discSystem.getBottom(), dur: (unit * 2) + "px", innerHTMLForFrontInner: `!settle able`});
                    disc.frontInnerDiv.addEventListener("click", (e) => {discSystem.getLoupe().innerHTML = e.target.innerHTML;});
                    disc.moveY(unit * 1);
                    disc.moveX(unit4Disc * 1.1);

                    disc = new Disc({rootDiv: discSystem.getBottom(), dur: (unit * 2) + "px", innerHTMLForFrontInner: `settle able  spoil able`});
                    disc.frontInnerDiv.addEventListener("click", (e) => {discSystem.getLoupe().innerHTML = e.target.innerHTML;});
                    disc.moveY(unit * 4);
                    disc.moveX(unit4Disc * 1.1);
                    ...
                    let historian;
                    historian = new Historian({rootDiv: discSystem.getBottom(), dur: (unit * 8) + "px", innerHTML: "gourd"});
                    historian.moveX(unit4Disc * 2.1);
...
/*
                    disc = new Disc({rootDiv: discSystem.getBottom(), innerHTMLForFrontInner: `
                        <font size="1px">
                            <font color="blue">縫: </font>lotVendor.rx << currentGame<br />
                            <font color="blue">纈: </font>hf.rx << currentGame<br />
                        </font>
                    `});
*/
